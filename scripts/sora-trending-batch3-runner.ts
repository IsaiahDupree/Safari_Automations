#!/usr/bin/env npx tsx
/**
 * Sora Trending Batch 3 Runner
 * Runs video generation for real Feb 2026 trends across niches
 * 
 * Usage:
 *   npx tsx scripts/sora-trending-batch3-runner.ts                    # Run all
 *   npx tsx scripts/sora-trending-batch3-runner.ts 5                   # Start from movie 5
 *   npx tsx scripts/sora-trending-batch3-runner.ts 5 2                 # Start from movie 5, part 2
 *   npx tsx scripts/sora-trending-batch3-runner.ts --trilogies         # Only trilogies
 *   npx tsx scripts/sora-trending-batch3-runner.ts --singles           # Only singles
 */
...
const CONFIG_FILE = path.join(__dirname, '..', 'sora-trending-batch-3.json');
...
async function main(): Promise<void> {
  console.log('╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║   🔥 SORA TRENDING BATCH 3 — @isaiahdupree                           ║');
  console.log('║   25 Scripts, 47 Videos — Real Feb 2026 Trends                       ║');
  console.log('║   Niches: TikTok Viral, Fitness, Finance, AI, Lifestyle, Comedy       ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝');
  ...
  const outputBaseDir = path.join(process.env.HOME || '', 'sora-videos', 'trending-batch-3');
  ...
  const progress: Record<string, any> = {
    ...
    config: 'sora-trending-batch-3.json',
    results: [],
  };
