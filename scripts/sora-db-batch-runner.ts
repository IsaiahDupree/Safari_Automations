 ...
 * 
 * Usage:
 *   npx tsx scripts/sora-db-batch-runner.ts                    # Run all
 *   npx tsx scripts/sora-db-batch-runner.ts 5                   # Start from movie 5
 *   npx tsx scripts/sora-db-batch-runner.ts 5 2                 # Start from movie 5, part 2
 *   npx tsx scripts/sora-db-batch-runner.ts --trilogies         # Only trilogies
 *   npx tsx scripts/sora-db-batch-runner.ts --singles           # Only singles
 */
...
const CONFIG_FILE = path.join(__dirname, '..', 'sora-db-generated-batch-1.json');
...
async function main(): Promise<void> {
  console.log('╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║   🔥 SORA DB-GENERATED BATCH — @isaiahdupree                        ║');
  console.log('║   10 Scripts, 20 Videos (5 trilogies + 5 singles)                    ║');
  ...
  const outputBaseDir = path.join(process.env.HOME || '', 'sora-videos', 'db-generated-batch-1');
  ...
  const progress: Record<string, any> = {
    ...
    config: 'sora-db-generated-batch-1.json',
    results: [],
  };
