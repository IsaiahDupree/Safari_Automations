#!/usr/bin/env npx tsx
/**
 * Sora February 2026 Trends Runner
 * 7 Scripts × 15 Total Videos (4 singles + 4 trilogies)
 * Trend-based content for @isaiahdupree
 * 
 * Usage:
 *   npx tsx scripts/sora-february-trends-runner.ts              # Run all
 *   npx tsx scripts/sora-february-trends-runner.ts 5             # Start from movie 5
 *   npx tsx scripts/sora-february-trends-runner.ts 5 2           # Start from movie 5, part 2
 *   npx tsx scripts/sora-february-trends-runner.ts --trilogies   # Only trilogies (2,4,5,7)
 *   npx tsx scripts/sora-february-trends-runner.ts --singles     # Only singles (1,3,6)
 */
...
const CONFIG_FILE = path.join(__dirname, '..', 'sora-february-2026-trends.json');
...
async function runMovie(movie: Movie, outputBaseDir: string): Promise<number> {
  ...
  if (movie.soundtrack) console.log(`   🎵 Sound: ${movie.soundtrack}`);
...
async function main(): Promise<void> {
  console.log('╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║   🔥 SORA FEBRUARY 2026 TRENDS — @isaiahdupree                      ║');
  console.log('║   7 Scripts, 15 Videos (4 singles + 4 trilogies)                     ║');
  console.log('║   Trend-aligned content for TikTok / Reels / Shorts                  ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝');

  const data: TrendsData = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf-8'));
  ...
  const outputBaseDir = path.join(process.env.HOME || '', 'sora-videos', 'february-2026-trends');
  ...
  const progress: Record<string, any> = {
    ...
    config: 'sora-february-2026-trends.json',
    results: [],
  };
  ...
  for (const movie of movies) {
    ...
    // If starting mid-movie, filter parts
  ...
  console.log('\n   Next steps:');
  console.log('   1. Run watermark cleaner on all generated videos');
  console.log('   2. Stitch trilogies with: ffmpeg -i part-1.mp4 -i part-2.mp4 -i part-3.mp4 ...');
  console.log('   3. Regenerate master catalog: npx tsx scripts/generate-sora-catalog.ts');
  console.log('\n🎬 February 2026 trends content ready!');
}
