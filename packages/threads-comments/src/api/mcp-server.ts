...
const TOOLS = [
  ...
  { name: 'threads_session_ensure', description: 'Compatibility operation: discover or open a Threads tab across all Safari windows; no claim is created.', inputSchema: { type: 'object', properties: {} } },
  { name: 'threads_claim_status', description: 'Compatibility status: global Safari claims are retired, so this reports open availability.', inputSchema: { type: 'object', properties: {} } },
  { name: 'threads_release_session', description: 'Compatibility no-op: no global Safari claim or lane exists to release.', inputSchema: { type: 'object', properties: {} } },
  {
    name: 'threads_comment_sweep',
    description: 'Batch comment sweep on Threads: search a keyword, generate AI comments for each matching post, and post them. Has a 5-minute timeout and discovers tabs across all Safari windows.',
    inputSchema: {
      type: 'object',
      properties: {
