...
export async function clickFallback(_selectors: string[], _timeoutMs = 5_000): Promise<string> { throw disabled(); }
export async function waitForSelectorFallback(_selectors: string[], _timeoutMs = 10_000): Promise<{ selector: string; found: boolean }> { throw disabled(); }
...
export async function getCDPStatus(): Promise<{ connected: boolean; hasTwitterTab: boolean; url: string; error?: string }> {
  return { connected: false, hasTwitterTab: false, url: '', error: 'Direct Chrome automation disabled' };
}
