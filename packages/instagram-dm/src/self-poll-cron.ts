/**
 * SelfPollCron for instagram-dm (SDPA-011)
 * =========================================
 * Runs inside the instagram-dm server process.
 * Fetches Instagram DM conversations and unread notifications,
 * then writes them to safari_platform_cache for cloud-sync pollers to consume.
 *
 * The server exposes POST /api/instagram/self-poll which the cron-manager fires.
...
export class SelfPollCron {
  ...
  async tick(): Promise<{ fetched: Record<string, number> }> {
    const db = getDb();
    if (!db) {
      console.warn('[SelfPoll:instagram] Supabase not configured — skipping cache write');
    ...
    try {
      ...
      const convoData = await fetchJson<{ conversations?: any[] }>(`${base}/api/conversations`);
      const conversations = convoData?.conversations || [];
      if (conversations.length > 0) {
        await writeCache(db, 'instagram', 'dms', conversations);
        fetched.dms = conversations.length;
        console.log(`[SelfPoll:instagram] Cached ${fetched.dms} DM conversations`);
      }
      ...
      if (notifications.length > 0) {
        await writeCache(db, 'instagram', 'notifications', notifications);
        fetched.notifications = notifications.length;
        console.log(`[SelfPoll:instagram] Cached ${fetched.notifications} notifications`);
      }
    } catch (e) {
      console.error('[SelfPoll:instagram] tick error:', (e as Error).message);
    }
  ...
  start(intervalMs = 300_000): void {
    ...
    this.intervalHandle = setInterval(() => {
      this.tick().catch(e => console.error('[SelfPoll:instagram] tick error:', e.message));
    }, intervalMs);
    console.log(`[SelfPoll:instagram] Started — polling every ${intervalMs / 60_000} min (24/7)`);
    this.tick().catch(e => console.error('[SelfPoll:instagram] initial tick error:', e.message));
  }
