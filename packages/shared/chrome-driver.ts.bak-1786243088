...
export class ChromeDriver {
  ...
  async getPage(): Promise<Page> {
    ...
    // Fall back to last tab or open a new one
    if (pages.length > 0) {
      this._page = pages[pages.length - 1];
      console.log(`[chrome-driver:${this.opts.platform}] Reusing last tab: ${this._page.url().slice(0, 80)}`);
      return this._page;
    }

    console.log(`[chrome-driver:${this.opts.platform}] Opening new tab`);
